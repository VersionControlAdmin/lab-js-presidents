...
function  countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((acc, crr) => acc + ((crr.leftOffice) ? crr.leftOffice - crr.tookOffice : 0), 0)
}
...
function getAgeAtInauguration(presidentsArr) {
  presidentsArr.map((element,index) => presidentsArr[index]["ageAtInauguration"] = element.tookOffice-element.birthYear);
  return presidentsArr;
}
...
function countRepublicanPresidents(presidentsArr) {
  return presidentsArr.reduce((acc, crr) => (crr.party==="Republican") ? acc + 1 : acc+0, 0);
}
